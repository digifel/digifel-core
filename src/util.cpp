...
const char * const BITCOIN_CONF_FILENAME = "digifel.conf";
const char * const BITCOIN_PID_FILENAME = "digifeld.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "digifel" is a composite category enabling all Digifel-related debug output
            if(ptrCategory->count(string("digifel"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DigifelCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DigifelCore
    // Mac: ~/Library/Application Support/DigifelCore
    // Unix: ~/.digifelcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DigifelCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DigifelCore";
#else
    // Unix
    return pathRet / ".digifelcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty digifel.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override digifel.conf
