 ...
 *
 *   // network
 *   CDigifelNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.digifel.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CDigifelNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CDigifelNetwork(UniValue objIn)
    {
...
// // can be under: DigifelNetwork
// //   -- signature requirements : Key1(User)
// class CDigifelNetworkVariable : public CGovernanceObject
// {
// private:
